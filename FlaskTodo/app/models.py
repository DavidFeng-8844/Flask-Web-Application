...
class Todo(db.Model):
    id = db.Column(db.Integer,
                   primary_key=True)  # Unique ID for each task
    title = db.Column(db.String(200),
                      nullable=False)  # Title of the Task
    module_code = db.Column(db.String(8),
                            nullable=False)  # Format like XJCO2011
    ...
    soft_delete = db.Column(db.Boolean,
                            default=False)  # Field for soft deletes
    ...
    def __repr__(self):
        return (
            f"Todo('{self.title}', '{self.module_code}', "
            f"'{self.completed}', '{self.deadline}', '{self.importance}')"
        )
